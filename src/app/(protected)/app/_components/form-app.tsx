...
function FormApp({ existingId }: { existingId?: number | null }) {
  ...
  const onSubmit = async (val: any) => {
    try {
      ...
      if (!existingId) {
        await BaseService.addApp(payload);
      } else {
        await BaseService.updateApp(existingId, payload);
      }
  ...
  useEffect(() => {
    if (existingId && app) {
      form.setFieldsValue({
        name: app.data.name,
        deskripsi: app.data?.deskripsi || "",
        kode: app.data?.kode || "",
        jenis: app.data?.jenis || "",
      });
      if (app.data.transaction_fee) {
        form.setFieldsValue({
          type_per_transaction: app.data.transaction_fee?.type_per_transaction,
          fee_per_transaction: app.data.transaction_fee?.fee_per_transaction,
          type_per_withdrawal: app.data.transaction_fee?.type_per_withdrawal,
      ...
      if (app.data.plan.length) {
        form.setFieldsValue({
          planName: app.data.plan[0]?.name,
          planDescription: app.data.plan[0]?.description,
          planHarga: app.data.plan[0]?.harga,
          planDuration: app.data.plan[0]?.duration,
          planDurationFinance: app.data.plan[0]?.duration_finance,
        });
      }
    }
  }, [existingId, app]);
